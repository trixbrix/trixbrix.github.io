...
(function() {
  ...
  const T = {
    en: {
      ...
      'detect.legacyDetected':'Older firmware detected (version reporting was added in v2). Your device will update normally — when the flashing tool asks <em>Erase device?</em>, leave the box unchecked to keep your saved settings.',
      'detect.unknownDevice': "This doesn't look like a Trixbrix controller. Make sure you picked the right device in the chooser.",
    ...
    de: {
      ...
      'detect.legacyDetected':'Ältere Firmware erkannt (Versionsmeldung wurde in v2 hinzugefügt). Dein Gerät wird normal aktualisiert — wenn das Flash-Tool nach <em>Gerät löschen?</em> fragt, lasse das Häkchen leer, um die gespeicherten Einstellungen zu behalten.',
      'detect.unknownDevice': 'Das sieht nicht nach einem Trixbrix-Controller aus. Stelle sicher, dass du das richtige Gerät ausgewählt hast.',
    ...
    fr: {
      ...
      'detect.legacyDetected':"Ancien firmware détecté (le rapport de version a été ajouté en v2). Votre appareil sera mis à jour normalement — quand l'outil de flash demande <em>Effacer l'appareil ?</em>, laissez la case décochée pour conserver vos paramètres enregistrés.",
      'detect.unknownDevice': "Cela ne ressemble pas à un contrôleur Trixbrix. Assurez-vous d'avoir choisi le bon appareil.",
    ...
    es: {
      ...
      'detect.legacyDetected':'Firmware antiguo detectado (el reporte de versión se añadió en v2). Tu dispositivo se actualizará normalmente — cuando la herramienta pregunte <em>¿Borrar dispositivo?</em>, deja la casilla sin marcar para conservar tus ajustes guardados.',
      'detect.unknownDevice': 'Esto no parece un controlador Trixbrix. Asegúrate de haber elegido el dispositivo correcto.',
    ...
    pl: {
      ...
      'detect.legacyDetected':'Wykryto starszy firmware (raportowanie wersji dodano w v2). Urządzenie zostanie zaktualizowane normalnie — gdy narzędzie zapyta <em>Erase device?</em>, pozostaw pole niezaznaczone, aby zachować zapisane ustawienia.',
      'detect.unknownDevice': 'To nie wygląda na kontroler Trixbrix. Upewnij się, że wybrałeś właściwe urządzenie.',
    ...
    nl: {
      ...
      'detect.legacyDetected':'Oudere firmware gedetecteerd (versierapportage is toegevoegd in v2). Je apparaat wordt normaal bijgewerkt — wanneer de flash-tool vraagt <em>Erase device?</em>, laat het vakje uitgevinkt om je opgeslagen instellingen te behouden.',
      'detect.unknownDevice': 'Dit lijkt niet op een Trixbrix-controller. Zorg dat je het juiste apparaat hebt gekozen.',
